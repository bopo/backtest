...
class Day:
    ...
    def __init__(self, path=None, export='csv'):
        path = path if path else os.path.abspath(os.path.expanduser("~/.quantity/bundle"))
    ...
    def init(self):
        ...
        # for stock_code in stock_codes:
            # self.init_stock_history(stock_code)
        pool = ThreadPool(10)
        pool.map(self.init_stock_history, stock_codes)

        with open(os.path.join(self.store.bundle_path,'initialized.lock')) as f:
            f.write('done')

        print('done')

    def bundle(self):
        lockfile = os.path.join(self.store.bundle_path, 'initialized.lock')
        self.update() if os.path.exists(lockfile) else self.init()
    ...
    def get_quarter_history(self, stock_code, year, quarter):
        ...
        print('==> {},{},{} ... '.format(stock_code, year, quarter))
